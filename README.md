...
Mac permissions: the first recording asks for microphone access. The hotkey registers through the system hotkey API (the one Spotlight uses), so it needs no Input Monitoring access and works in every app, Terminal included. Pinning (below) needs Accessibility. A pin that sends into Terminal also needs Automation access to Terminal, which macOS asks for the first time you use one there.
...
In a terminal (Terminal, iTerm2, Ghostty, Warp, kitty, Alacritty, WezTerm) the marker is swapped with keystrokes. When the transcript arrives and the terminal window is in front, ClipBridge reads the screen text and the cursor position, walks the cursor back to the marker with the left arrow, and checks on screen that the cursor sits right after the marker. Only then does it backspace the marker away, type the transcript, and walk the cursor forward again. This works in a shell and in Claude Code's input box, across wrapped lines. A terminal that does not report its text and cursor (or a window that stays in the background for 90 seconds) keeps the marker, the pill reads "ready for ⟦note 1⟧", and the hook below covers Claude Code. Every pin is written to `~/.clipbridge/pins/<number>.json`, and [`mac/pin_hook.py`](mac/pin_hook.py), registered as a Claude Code `UserPromptSubmit` hook, finds the markers in a sent prompt, waits up to 110 seconds for any still transcribing, and gives Claude each transcript as what its marker stands for. The prompt text is left as typed.

Pinning and sending: the send hotkey (default ctrl+alt+return, `pin_send_hotkey` in the config) pins the note the same way and presses Return once the transcript is in, so a prompt you dictate into Claude Code, or into a chat box, goes out when it is ready. You can switch to another window or app straight after pressing it. Return is pressed only after the transcript reads back where the marker was, and the pill then reads "sent", or "pasted, not sent" when Return could not be pressed safely.

In Terminal this does not use keystrokes. ClipBridge notes the tab's tty when you pin, and when the transcript is ready it writes to that tab through AppleScript (`do script ... in tab`), which types into the tab as if at the keyboard and adds a Return, whether the window is in front, behind another, or on another Space. It reads the tab's screen the same way first, and types nothing until the marker is the last thing in Claude Code's input box and the box has held still for half a second. It keeps checking for up to ten minutes, which covers Claude Code asking a question that hides the box. Then one burst presses Control E, backspaces the marker away, and types the transcript, and the Return that follows sends it. Claude Code takes a burst of more than about 800 characters as a paste: it hands that text to Claude as pasted content rather than as your words, drops anything typed after a backspace in the same burst, and turns the Return into a new line. So a long transcript goes in as pieces of up to 400 bytes, each but the last ending in a backslash (Claude Code's own backslash Return new line), with the next piece starting by deleting that new line. Afterwards the input box is read again, and Return is pressed up to twice more if the prompt is still sitting in it. If you typed after the marker, the cursor could be anywhere, so the marker is left in place, Return is pressed, and the hook fills the marker in. Other terminals, and a text field in any app, get a Return keystroke when their window is in front with the field focused. A text field in an app that is behind others gets the Return posted to that app alone, and only while the field is still what has focus inside it; the field is then read back to see whether it let go of the text, as a chat box does when it sends.

`build.sh` signs the app with the Apple Development certificate when there is one, because an ad hoc signature changes on every build and macOS drops the Accessibility grant with it. While recording the menu bar icon turns into a red dot; while transcribing, an ellipsis.
...
| `pin_hotkey` | no | Mac, pin the newest note to the focused text field, default `<ctrl>+<alt>+v` |
| `pin_send_hotkey` | no | Mac, pin the newest note and press Return once it is in, default `<ctrl>+<alt>+return` |
